defmodule Mastery.Boundary.TemplateValidator do
  ...
  def errors(fields) when is_list(fields) do
    ...
    []
    |> V.require(fields, :name, &validate_name/1)
    |> V.require(fields, :category, &validate_name/1)
    |> V.optional(fields, :instructions, &validate_instructions/1)
    |> V.require(fields, :raw, &validate_raw/1)
    |> V.require(fields, :generators, &validate_generators/1)
    |> V.require(fields, :checker, &validate_checker/1)
  end
  ...
  def validate_raw(raw) when is_binary(raw) do
    V.check(String.match?(raw, ~r{\S}), {:error, "can't be blank"})
  end
  ...
  def validate_generator({name, generator})
    ...
    V.check(generator != [], {:error, "can't be empty"})
  end
